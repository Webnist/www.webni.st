...
async function wpFetchJson<T>(url: string, revalidateSec = 60): Promise<T> {
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `WP menu fetch failed: ${res.status} ${res.statusText} url=${url} body=${text.slice(0, 200)}`
    );
  }
...
/**
 * WordPress のカスタム REST API からメニューを取得します。
 * @param args - 引数
 * @returns メニューレスポンス
 * @throws エラー
 */
export async function getMenu(args: GetMenuArgs): Promise<MenuResponse> {
  const url = wpUrl('/wp-json/webnist/v1/menu', {
    site: args.siteId,
    area: args.area,
  });

  return wpFetchJson<MenuResponse>(url, 60);
}
